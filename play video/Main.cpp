                     ...
                     int       nCmdShow)
{
	...
	sscanf(lpCmdLine, "%s %d %d %d", &ImagePath, &w, &h, &FPS);
	inImage.setWidth(w);
	inImage.setHeight(h);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   hWnd = CreateWindow(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW,
      600, 200, inImage.getWidth()+25, inImage.getHeight()+140, NULL, NULL, hInstance, NULL);
		...
		case WM_PAINT:
			{
				...
				char text[1000];
				sprintf(text, "Video wid:%d, hgt:%d\n", inImage.getWidth(), inImage.getHeight());
				//strcpy(text, "Video wid:%d, height:%d\n");
