...
int N = 0;	// The vectors length as input
bool isColor; // Choose different color mode
                     ...
                     int       nCmdShow)
{
	...
	int w=352, h=288;	// Hardcoded the width and height
	char ImagePath[_MAX_PATH];
	sscanf(lpCmdLine, "%s %d", &ImagePath, &N);
	inImage.setWidth(w);
	inImage.setHeight(h);
	if (strstr(ImagePath, ".rgb") == NULL && strstr(ImagePath, ".raw") == NULL)
	{
		AfxMessageBox("Image has to be a '.rgb/.raw' file\nUsage - Image.exe image.rgb/raw N");
		//return FALSE;
	}
	// Read color .rgb image
	else if (strstr(ImagePath, ".rgb") != NULL)
	{
		inImage.setImagePath(ImagePath);
		if (!inImage.ReadImage())
		{
			AfxMessageBox("Could not read image\nUsage - Image.exe image.rgb N");
			//return FALSE;
		}
		else {
			outImage = inImage;
	...
	// Read grayscale .raw image
	else if (strstr(ImagePath, ".raw") != NULL)
	{
		inImage.setImagePath(ImagePath);
		if (!inImage.ReadGrayImage())
		{
			AfxMessageBox("Could not read image\nUsage - Image.exe image.raw N");
			//return FALSE;
		}
		else {
			outImage = inImage;
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   hWnd = CreateWindow(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW,
	   580, 250, 780, 450, NULL, NULL, hInstance, NULL);
		...
		case WM_PAINT:
			{
				...
				int error;
				if (isColor)
					error=inImage.Modify(N);
				else
					error=inImage.ModifyGray(N);
				//Display modified image
				SetDIBitsToDevice(hdc,
								  380, 50, inImage.getWidth(), inImage.getHeight(),
								  0, 0, 0, inImage.getHeight(),
								  inImage.getImageData(), &bmi, DIB_RGB_COLORS);
